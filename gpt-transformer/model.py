...
from scripts import LinearlyCompressedDecoder, ConvCompressedDecoder
...
class GPT(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor]:
        ...
        logits = self.cls_net(decoder_out) # B, K, OUTPUT_VOCAB_SIZE
...
class ParallelGPT(nn.Module):
    ...
    def forward(self, x: torch.Tensor, drop_one_block: bool=False, y: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor]:
...
class LinearlyCompressedGPT(nn.Module):
    ...
    def __init__(self, config: Dict[str, Union[str, int]]) -> None:
        ...
        self.input_embedding = Embedding(config)
        self.lc_decoder = LinearlyCompressedDecoder(config=config)
        self.cls_net = nn.Sequential(
            nn.Dropout(config["dropout"]),
...
class ConvCompressedGPT(nn.Module):
    ...
    def __init__(self, config: Dict[str, Union[str, int]]) -> None:
        ...
        d_model = config["d_model"]
        self.input_embedding = Embedding(config)
        self.cc_decoder = ConvCompressedDecoder(config=config)
        self.cls_net = nn.Sequential(
            nn.Dropout(config["dropout"]),
            nn.Linear(d_model//(2**(config["n_layers"]//2)),
                      config["vocab_size"])
